...
export default function RegisterForm({ termsContent }: RegisterFormProps) {
  const router = useRouter();
  const [formData, setFormData] = useState({
    firstName: '', lastName: '', email: '', phone: '', countryCode: '+90',
    password: '', confirmPassword: '', termsAccepted: false, role: 'student' as 'student' | 'teacher'
  });
  ...
  const handlePhoneKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    // Allow: backspace, delete, tab, escape, enter, ctrl+a, home, end, arrows
  ...
  const updateField = (field: string, value: string | boolean) => {
    setFormData(prev => {
      ...
      // When main password changes, re-check confirm field
      if (field === 'password') {
        ...
        // Also clear the password error if it was set
  ...
  const isFormValid = () => {
    return formData.firstName.trim() && formData.lastName.trim() &&
      formData.email.trim() && formData.phone.trim() && formData.password.length >= 6 &&
      ...
      <main className="min-h-screen flex items-center justify-center py-12 px-4 bg-brand-primary-light">
        <div className="w-full max-w-[500px] bg-white rounded-lg border border-gray-200 shadow-xl p-6 md:p-8">
          ...
          <div className="flex flex-col items-center mb-6">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-brand-primary mb-2">Aramıza Katılın</h1>
            <p className="text-sm text-gray-600">Yeni bir dil, yeni bir dünya. Hemen kaydolun.</p>
          </div>
          ...
          {success && (
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg animate-in fade-in duration-300">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-sm text-green-700 font-semibold mb-1">Kayıt başarılı!</p>
                  <p className="text-sm text-green-600">
                    E-posta adresinize bir doğrulama bağlantısı gönderdik. Lütfen e-postanızı kontrol edin ve hesabınızı aktifleştirin.
                  </p>
                  <Link href="/giris" className="inline-block mt-3 text-sm text-green-700 font-semibold hover:underline">
                    → Giriş sayfasına git
                  </Link>
                </div>
              </div>
          ...
          {!success && (
            <form onSubmit={handleSubmit} className="space-y-4">
              {/* Role Selection - Removed for students only */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {errors.confirmPassword && (
                    <p className="text-red-600 text-xs mt-1.5 flex items-center gap-1.5 animate-in fade-in duration-200">
                ...
                <div className="space-y-1">
                  <div className="flex gap-1">
                    {[1, 2, 3, 4].map((level) => (
                      <div
                        key={level}
                        className={`h-1 flex-1 rounded-full transition-colors ${formData.password.length >= level * 3
                  ...
                  <p className="text-xs text-gray-500">
                    {formData.password.length < 6 ? 'Çok kısa' :
                      formData.password.length < 8 ? 'Zayıf' :
                        formData.password.length < 12 ? 'Orta' : 'Güçlü'}
                  </p>
                </div>
              )}
              ...
              <div className="pt-2">
                ...
                {errors.terms && (
                  <p className="text-red-600 text-xs mt-1.5 flex items-center gap-1 ml-6">
                    <span className="inline-block w-1 h-1 bg-red-600 rounded-full"></span>
                    {errors.terms}
                  </p>
                )}
              ...
              <button
                ...
                className={`w-full py-3 rounded-lg font-semibold transition-all text-base mt-5 flex items-center justify-center gap-2 ${isFormValid() && !loading
                  ? 'bg-brand-primary text-white hover:bg-brand-primary-hover shadow-md hover:shadow-lg cursor-pointer'
